$(document).ready(function() {
    ...
    $.ajax({
      ...
      window.alert("There was a problem getting the departures, try again later!");
    });
    ...
    function populateTable(info) {
      ...
      if (info != undefined && info != null && info[0] != []) {
        ...
        let table = document.getElementById("departures_table");
        ...
        for (let i = 0; i < info.length; i++) {
          ...
          relevantInfo.push(
            info[i].id,
            info[i].departure.date,
            info[i].departure.location,
            info[i].departure.time_local,
            info[i].time,
